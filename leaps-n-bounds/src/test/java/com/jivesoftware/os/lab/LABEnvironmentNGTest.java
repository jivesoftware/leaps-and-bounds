...
public class LABEnvironmentNGTest {
    ...
    public void testEnv() throws Exception {
        ...
        try {
            ...
            ValueIndex index = env.open("foo", 4096, 1000, -1, -1, -1, new LABRawhideTemp());
            indexTest(index);

            env = new LABEnvironment(LABEnvironment.buildLABCompactorThreadPool(4), LABEnvironment.buildLABDestroyThreadPool(1), root,
                true, 4, 8, 8);

            index = env.open("foo", 4096, 1000, -1, -1, -1, new LABRawhideTemp());
            indexTest(index);
            ...
            env = new LABEnvironment(LABEnvironment.buildLABCompactorThreadPool(4), LABEnvironment.buildLABDestroyThreadPool(1), root,
                true, 4, 8, 8);
            env.rename("foo", "bar");
            index = env.open("bar", 4096, 1000, -1, -1, -1, new LABRawhideTemp());
